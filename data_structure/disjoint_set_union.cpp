...
void join(int u, int v) {
    ...
    if(s[u] >= s[v]) swap(u, v);
    parent[u] = v;
    s[v] += s[u]; // perceba que só os representantes de cada conjunto disjunto é que vão guardar a informação do tamanho do conjunto.
...
// como a otimização pelo tamanho dos conjuntos nem sempre é necessária, pode-se usar esse código com frequência..
void join(int u, int v) {
    u = root(u);
    v = root(v);
    parent[u] = v;
